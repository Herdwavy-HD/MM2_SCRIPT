...
local seeds = ("Carrot,Strawberry,Blueberry,Tulip,Tomato,Apple,Corn,Bamboo,Cactus,Baby Cactus,Pineapple,Mushroom,Green Bean,Banana,Grape,Coconut,Mango,Dragon Fruit,Acorn,Cherry,Sunflower,Briar Rose,Venus Fly Trap,Pomegranate,Poison Apple,Devils Eye,Moon Bloom,Hypno Bloom,Dragons Breath"):split(",")
local sel = {}

local function applySelf()
    if not sEsp or not lP.Character then return end
    if not lP.Character:FindFirstChild("SelfChams") then
        local h = Instance.new("Highlight", lP.Character)
        h.Name = "SelfChams"
...
local function applyPlayer(p)
    if not pEsp or p == lP or not p.Character then return end
    if not p.Character:FindFirstChild("PlayerChams") then
...
local function toggPlayer(st)
    pEsp = st
    if pEsp then
        pHb = R.Heartbeat:Connect(function()
            for _, p in pairs(P:GetPlayers()) do pcall(applyPlayer, p) end
        end)
    else
        if pHb then pHb:Disconnect() pHb = nil end
        for _, p in pairs(P:GetPlayers()) do
            if p.Character and p.Character:FindFirstChild("PlayerChams") then p.Character.PlayerChams:Destroy() end
        end
    end
end
...
for _, n in pairs(seeds) do 
    ...
    local cb = Instance.new("TextButton", r) cb.Size = UDim2.new(0, 48, 0, 26) cb.Position = UDim2.new(1, -65, 0.5, -13) cb.BackgroundColor3 = Color3.fromRGB(45, 45, 50) cb.Text = "" cb.ZIndex = 50 Instance.new("UICorner", cb).CornerRadius = UDim.new(1, 0) 
    local d = Instance.new("Frame", cb) d.Size = UDim2.new(0, 18, 0, 18) d.Position = UDim2.new(0, 3, 0.5, -9) d.BackgroundColor3 = Color3.fromRGB(255, 255, 255) d.ZIndex = 51 Instance.new("UICorner", d).CornerRadius = UDim.new(1, 0)
    cb.MouseButton1Down:Connect(function() 
        sel[n] = not sel[n] 
        if sel[n] then cb.BackgroundColor3 = Color3.fromRGB(255, 30, 30) d:TweenPosition(UDim2.new(1, -21, 0.5, -9), 0, 0, 0.12) else cb.BackgroundColor3 = Color3.fromRGB(45, 45, 50) d:TweenPosition(UDim2.new(0, 3, 0.5, -9), 0, 0, 0.12) end 
    end)
end
...
local function makeVisualRow(name, yPos, callback)
    local card = Instance.new("Frame", P2) card.Size = UDim2.new(1, 0, 0, 55) card.Position = UDim2.new(0, 0, 0, yPos) card.BackgroundColor3 = Color3.fromRGB(22, 22, 26) card.ZIndex = 15 Instance.new("UICorner", card).CornerRadius = UDim.new(0, 8)
    local lbl = Instance.new("TextLabel", card) lbl.Size = UDim2.new(0, 230, 1, 0) lbl.Position = UDim2.new(0, 15, 0, 0) lbl.BackgroundTransparency = 1 lbl.Text = name lbl.TextColor3 = Color3.fromRGB(230, 230, 235) lbl.Font = Enum.Font.GothamMedium lbl.TextSize = 13 lbl.TextXAlignment = Enum.TextXAlignment.Left lbl.ZIndex = 16
    local vs = Instance.new("TextButton", card) vs.Size = UDim2.new(0, 48, 0, 26) vs.Position = UDim2.new(1, -65, 0.5, -13) vs.BackgroundColor3 = Color3.fromRGB(45, 45, 50) vs.Text = "" vs.ZIndex = 100 Instance.new("UICorner", vs).CornerRadius = UDim.new(1, 0)
    local vd = Instance.new("Frame", vs) vd.Size = UDim2.new(0, 18, 0, 18) vd.Position = UDim2.new(0, 3, 0.5, -9) vd.BackgroundColor3 = Color3.fromRGB(255, 255, 255) vd.ZIndex = 101 Instance.new("UICorner", vd).CornerRadius = UDim.new(1, 0)
    local active = false
    vs.MouseButton1Down:Connect(function()
        active = not active callback(active)
        if active then vs.BackgroundColor3 = Color3.fromRGB(255, 30, 30) vd:TweenPosition(UDim2.new(1, -21, 0.5, -9), 0, 0, 0.12) else vs.BackgroundColor3 = Color3.fromRGB(45, 45, 50) vd:TweenPosition(UDim2.new(0, 3, 0.5, -9), 0, 0, 0.12) end
    end)
end

makeVisualRow("Self ESP (White Chams)", 5, function(s) toggSelf(s) end)
makeVisualRow("Player ESP (Red Chams)", 65, function(s) toggPlayer(s) end)
