...
local sE, pE, sH, pH, iJ, nc, aA = false, false, nil, nil, false, false, false

local function ch(target, name, color, trans)
    if target and not target:FindFirstChild(name) then
        local hl = Instance.new("Highlight", target)
        hl.Name = name hl.FillColor = color hl.FillTransparency = trans
        hl.OutlineColor = color hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
    end
end

local function tS(st)
    sE = st
    if sE then sH = R.Heartbeat:Connect(function() pcall(ch, lP.Character, "SelfChams", Color3.new(1,1,1), 0.4) end)
    else if sH then sH:Disconnect() sH = nil end
        if lP.Character and lP.Character:FindFirstChild("SelfChams") then lP.Character.SelfChams:Destroy() end
    end
end

local function tP(st)
    pE = st
    if pE then pH = R.Heartbeat:Connect(function() for _,p in pairs(P:GetPlayers()) do if p ~= lP and p.Character then pcall(ch, p.Character, "PlayerChams", Color3.new(1,0,0), 0.6) end end end)
    else if pH then pH:Disconnect() pH = nil end
        for _,p in pairs(P:GetPlayers()) do if p.Character and p.Character:FindFirstChild("PlayerChams") then p.Character.PlayerChams:Destroy() end end
    end
end
U.JumpRequest:Connect(function()
    if iJ and lP.Character and lP.Character:FindFirstChildOfClass("Humanoid") then
...
local mC = Instance.new("UICorner", MainFrame) mC.CornerRadius = UDim.new(0, 14) Instance.new("UIStroke", MainFrame).Color = Color3.fromRGB(255, 30, 30)
...
createWinButton("🗖", 48, Color3.new(0.8, 0.8, 0.8), function()
    ...
    if isMaximized then MainFrame.Size = UDim2.new(1, 0, 1, 0) MainFrame.Position = UDim2.new(0, 0, 0, 0) mC.CornerRadius = UDim.new(0, 0)
    else MainFrame.Size = UDim2.new(0, 640, 0, 420) MainFrame.Position = UDim2.new(0.5, -310, 0.5, -210) mC.CornerRadius = UDim.new(0, 14) end
end)
...
local pSeeds = createPage("Seeds Shop", 1) local pCrates = createPage("Crates Shop", 2) local pGear = createPage("Gear Shop", 3) local pVis = createPage("Visuals ESP", 4) local pTweaks = createPage("Player Tweaks", 5) local pFun = createPage("Server Fun", 6)

local function fillList(p, list, infoText)
    local SL = Instance.new("ScrollingFrame", p) SL.Size = UDim2.new(1, 0, 1, -65) SL.Position = UDim2.new(0, 0, 0, 5) SL.BackgroundColor3 = Color3.fromRGB(18, 18, 22) SL.BorderSizePixel = 0 SL.CanvasSize = UDim2.new(0, 0, 0, #list * 32) SL.ScrollBarThickness = 4 SL.ScrollBarImageColor3 = Color3.fromRGB(255, 30, 30) SL.ZIndex = 15 Instance.new("UICorner", SL).CornerRadius = UDim.new(0, 8) Instance.new("UIListLayout", SL).SortOrder = Enum.SortOrder.LayoutOrder
    for _, itemName in pairs(list) do
        local row = Instance.new("Frame", SL) row.Size = UDim2.new(1, -10, 0, 30) row.BackgroundTransparency = 1 row.ZIndex = 16
        local lbl = Instance.new("TextLabel", row) lbl.Size = UDim2.new(1, -50, 1, 0) lbl.Position = UDim2.new(0, 10, 0, 0) lbl.BackgroundTransparency = 1 lbl.Text = itemName lbl.TextColor3 = Color3.fromRGB(230, 230, 235) lbl.Font = Enum.Font.GothamMedium lbl.TextSize = 11 lbl.TextXAlignment = Enum.TextXAlignment.Left lbl.ZIndex = 17
        local cb = Instance.new("TextButton", row) cb.Size = UDim2.new(0, 44, 0, 22) cb.Position = UDim2.new(1, -60, 0.5, -11) cb.BackgroundColor3 = Color3.fromRGB(45, 45, 50) cb.Text = "" cb.ZIndex = 50 Instance.new("UICorner", cb).CornerRadius = UDim.new(1, 0)
        local dot = Instance.new("Frame", cb) dot.Size = UDim2.new(0, 14, 0, 14) dot.Position = UDim2.new(0, 3, 0.5, -7) dot.BackgroundColor3 = Color3.new(1, 1, 1) dot.ZIndex = 51 Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
        local active = false cb.MouseButton1Down:Connect(function() active = not active cb.BackgroundColor3 = active and Color3.fromRGB(255, 30, 30) or Color3.fromRGB(45, 45, 50) dot:TweenPosition(UDim2.new(0, active and 27 or 3, 0.5, -7), 0, 0, 0.12) end)
    end
    local Inp = Instance.new("TextBox", p) Inp.Size = UDim2.new(0, 70, 0, 30) Inp.Position = UDim2.new(0, 0, 1, -32) Inp.BackgroundColor3 = Color3.fromRGB(22, 22, 26) Inp.Text = "10" Inp.TextColor3 = Color3.new(1, 1, 1) Inp.Font = Enum.Font.GothamMedium Inp.TextSize = 12 Inp.ZIndex = 25 Instance.new("UICorner", Inp).CornerRadius = UDim.new(0, 6)
    local ActionBtn = Instance.new("TextButton", p) ActionBtn.Size = UDim2.new(1, -80, 0, 30) ActionBtn.Position = UDim2.new(0, 80, 1, -32) ActionBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 50) ActionBtn.Text = infoText ActionBtn.TextColor3 = Color3.fromRGB(150, 150, 150) ActionBtn.Font = Enum.Font.GothamBold ActionBtn.TextSize = 10 ActionBtn.ZIndex = 25 Instance.new("UICorner", ActionBtn).CornerRadius = UDim.new(0, 6)
end
...
createToggleRow(pVis, "Self ESP (White Chams)", 5, toggleSelfESP) createToggleRow(pVis, "Player ESP (Red Chams)", 55, togglePlayerESP)
createToggleRow(pTweaks, "Anti-AFK (Защита от афк киков)", 5, function(s) antiAfkEnabled = s end) createToggleRow(pTweaks, "Infinite Jump (Бесконечный прыжок)", 55, function(s) infJump = s end) createToggleRow(pTweaks, "Noclip (Хождение сквозь стены)", 105, function(s) noclip = s end)
...
createSliderRow(pTweaks, "WalkSpeed (Скорость)", 155, 16, 250, 16, function(v) if localPlayer.Character and localPlayer.Character:FindFirstChildOfClass("Humanoid") then localPlayer.Character:FindFirstChildOfClass("Humanoid").WalkSpeed = v end end)
createSliderRow(pTweaks, "JumpPower (Прыжок)", 205, 50, 350, 50, function(v) if localPlayer.Character and localPlayer.Character:FindFirstChildOfClass("Humanoid") then localPlayer.Character:FindFirstChildOfClass("Humanoid").JumpPower = v localPlayer.Character:FindFirstChildOfClass("Humanoid").UseJumpPower = true end end)
...
createActionButton(pFun, "Rejoin (Быстрый перезаход на сервер)", 5, function() if #P:GetPlayers() <= 1 then T:Teleport(game.PlaceId, localPlayer) else T:TeleportToPlaceInstance(game.PlaceId, game.JobId, localPlayer) end end)
createActionButton(pFun, "Server Hop (Прыгнуть на другой сервер)", 50, function() local sf = {} local x = H:JSONDecode(game:HttpGet("https://roblox.com"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=20")) for _, s in pairs(x.data) do if s.playing < s.maxPlayers and s.id ~= game.JobId then table.insert(sf, s.id) end end if #sf > 0 then T:TeleportToPlaceInstance(game.PlaceId, sf[math.random(1, #sf)], localPlayer) else T:Teleport(game.PlaceId, localPlayer) end end)
...
local Tgl = Instance.new("TextButton", Gui) Tgl.Size = UDim2.new(0, 45, 0, 45) Tgl.Position = UDim2.new(0, 15, 0, 15) Tgl.BackgroundColor3 = Color3.fromRGB(15, 15, 18) Tgl.Text = "H" Tgl.TextColor3 = Color3.fromRGB(255, 30, 30) Tgl.Font = 4 Tgl.TextSize = 22 Tgl.ZIndex = 100 local tSt = Instance.new("UIStroke", Tgl) tSt.Color = Color3.fromRGB(255, 30, 30) tSt.Thickness = 2 Instance.new("UICorner", Tgl).CornerRadius = UDim.new(1, 0) Tgl.MouseButton1Down:Connect(function() MainFrame.Visible = not MainFrame.Visible Tgl.TextColor3 = MainFrame.Visible and Color3.fromRGB(255, 30, 30) or Color3.fromRGB(120, 120, 120) tSt.Color = MainFrame.Visible and Color3.fromRGB(255, 30, 30) or Color3.fromRGB(50, 50, 50) end)

elseif game.PlaceId == 142823291 or game.PlaceId == 66654135 or game.PlaceId == 11217730461 then
print("--- MM2 MOD LOADED ---") local function gC(p) if p.Backpack:FindFirstChild("Knife") or (p.Character and p.Character:FindFirstChild("Knife")) then return Color3.new(1,0,0) elseif p.Backpack:FindFirstChild("Gun") or (p.Character and p.Character:FindFirstChild("Gun")) then return Color3.new(0,0,1) end return Color3.new(0,1,0) end
R.Heartbeat:Connect(function() for _,p in pairs(P:GetPlayers()) do if p ~= localPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then local c = gC(p) if not p.Character:FindFirstChild("RoleHighlight") then local hl = Instance.new("Highlight", p.Character) hl.Name = "RoleHighlight" hl.FillColor = c hl.FillTransparency = 0.5 hl.OutlineColor = c hl.DepthMode = 0 else p.Character.RoleHighlight.FillColor = c p.Character.RoleHighlight.OutlineColor = c end end end end)
task.spawn(function() while task.wait(0.2) do pcall(function() if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then for _,c in pairs(workspace:GetDescendants()) do if c.Name == "Coin_C" and c:IsA("BasePart") then localPlayer.Character.HumanoidRootPart.CFrame = c.CFrame task.wait(0.05) end end end end) end end)
end
