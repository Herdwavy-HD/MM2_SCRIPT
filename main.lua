print("--- HERDWAVY'S FIX MULTI-SEED SHOP LAUNCHED ---")
...
-- ТОЧНЫЙ СЕТЕВОЙ ТРИГГЕР ДЛЯ GROW A GARDEN 2
    ...
    task.spawn(function()
        for i = 1, _G.BuyAmount do
            ...
            for seedName, isSelected in pairs(selectedSeeds) do
                if isSelected then
                    pcall(function() buyRemote:FireServer(seedName, 1) end)
                end
            end
            task.wait(0.05)
        end
...
ScrollList.ZIndex = 10 -- ПОДНЯЛИ НА САМЫЙ ВЕРХНИЙ СЛОЙ
...
-- 📋 ТЕКСТ-ИНДИКАТОР БЕЗОПАСНОСТИ (ПОКАЗЫВАЕТ ЧТО КУПИТСЯ)
...
local function updateCountText()
    ...
    if c > 0 then
        ...
        SafeLabel.TextColor3 = Color3.fromRGB(255, 200, 0) -- Желтый цвет предупреждения
    else
        SafeLabel.Text = "Ничего не выбрано"
        SafeLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
    end
...
-- Создание строчек с высоким ZIndex
for _, seedName in pairs(seedList) do
    ...
    SeedNameLabel.TextColor3 = Color3.fromRGB(255, 255, 255) -- СДЕЛАЛИ БЕЛЫМ И ЯРКИМ
...
ListToggleBtn.MouseButton1Click:Connect(function()
    ...
    if ScrollList.Visible then
        MainFrame.Size = UDim2.new(0, 320, 0, 340) -- Увеличили высоту под список и инфо-текст
