...
-- ==========================================================
-- ЧАСТЬ 2: ХАКИ ДВИЖЕНИЯ И ИДЕАЛЬНЫЕ ПРЯМОУГОЛЬНЫЕ КНОПКИ WINDOWS
-- ==========================================================
...
-- ВОЗВРАЩАЕМ УЗКИЙ, НИЗКИЙ ПРЯМОУГОЛЬНЫЙ ВИД И УБИРАЕМ К САМОМУ КРАЮ ВВЕРХ КНОПКИ WINDOWS
local ControlBox = Instance.new("Frame", MainFrame)
ControlBox.Size = UDim2.new(0, 150, 0, 20) 
ControlBox.Position = UDim2.new(1, -165, 0, 4) -- Подняли впритык к верхней красной линии рамки!
...
local function createWinButton(text, offset, color, callback)
    ...
    btn.Size = UDim2.new(0, 44, 0, 16) -- Те самые идеальные прямоугольники (низкие и широкие)
...
-- СДВИГ СЛАЙДЕРОВ НИЖЕ ИЗ-ЗА АНТИ-АФК
...
-- НАПОЛНЯЕМ ВКЛАДКУ SERVER FUN
createActionButton(pFun, "Rejoin (Быстрый перезаход на server)", 5, function()
